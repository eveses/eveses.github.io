...
<h3>EBS Volume Type</h3>

| 타입 | 분류 | 특성/성능 | 주요 용도 |
| :--- | :--- | :--- | :--- |
| gp2 | SSD (범용) | 용량(GB)에 비례해 IOPS 증가, 최대 16,000 IOPS | 일반 워크로드, 부팅 디스크 |
| gp3 | SSD (범용) | 기본 3,000 IOPS / 125 MB/s, 최대 16,000 IOPS / 1,000 MB/s | 대부분의 신규 워크로드, gp2 대비 저렴 |
| io1 | SSD (프로비저닝) | 높은 IOPS 프로비저닝, 일관된 지연시간, Multi-Attach 지원 | 중요/고성능 DB, 트랜잭션 워크로드 |
| io2 | SSD (프로비저닝) | 더 높은 내구성(미션 크리티컬), 고 IOPS, Multi-Attach 지원 | 미션 크리티컬 DB, 초고성능 워크로드 |
| st1 | HDD (Throughput) | 대용량, 순차 처리에 최적, 높은 처리량, 낮은 IOPS | 로그, 빅데이터, 데이터 웨어하우스 |
...
> - **"Windows 인스턴스 간 파일 공유가 필요하다"** -> **FSx for Windows** 
