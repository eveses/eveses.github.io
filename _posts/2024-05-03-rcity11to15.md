...
그럼 /usr/bin/find 를 rcity15 유저의 권한으로 패스워드 없이 실행 가능하다고 나온다. <br> 이를 이용하기 위해 gtfobins (https://gtfobins.github.io/)를 찾아보자. <br> 이 사이트는 바이너리 파일을 이용해 우회하는 법들을 정리한 사이트이다. <br> 우리가 이용가능한 바이너리 파일은 find 이므로 찾아보면 sudo권한으로 find를 실행 가능할때 우회할 수 있는 코드가 나온다.
